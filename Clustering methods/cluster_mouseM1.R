...
library(parallel)
library(CYANO)
file_path_root = "/root/datasets/mouseM1/"
...
write.csv(pbmc@meta.data, paste0(file_path_root, "data1/seurat_result_5.csv"))
...
pbmc <- FindNeighbors(pbmc, dims = 1:14)
pbmc <- FindClusters(pbmc, resolution = 5)
write.csv(pbmc@meta.data, paste0(file_path_root, "data2/seurat_result_5.csv"))
...
k = estimate_k(dat,
               ...
               criteria_method = "NMI",
               krange = 100:150, ensemble_sizes = 10,
               cores = 1
)
...
saveRDS(cluster,paste0("data1/scCCESS_simlr.rds"))
end <- Sys.time()
runningtime <- end-start
...
saveRDS(cluster,paste0("data2/scCCESS_simlr.rds"))
